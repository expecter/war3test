...
export default class AppTest {
    constructor() {
        if (isDebug) {
            ...
            SolarDamageState.config.damageEventHandlers.push(event => {
                ...
                TextTagUtil.text(tag + Math.floor(event.resultDamage), event.unit0, 8, 2, 200);
            });
        }
    }
    onUnitAttacked(triggerUnit, attacker) {
        ...
        se.playerChat("3", () => {
            ...
            BaseUtil.onTimer(0.05, c => {
                ...
                const tween = new Tween({ ...screenCoordinates, p: 0, s: 0.04 })
                    ...
                    .onUpdate((temp) => {
                    //贝塞尔插值
                    let position = InterpolationUtil.bezier(temp.p, new Vector3(screenCoordinates.x, screenCoordinates.y), new Vector3(0, 0), new Vector3(0, 0.8), new Vector3(0.6, 0.8), new Vector3(0.6, 0), new Vector3(0, 0), new Vector3(0, 0.8), new Vector3(0.6, 0.8), new Vector3(0.6, 0));
    ...
    uiTest() {
        ...
        frame.setOnClick(() => {
            DisplayTimedTextToPlayer(GetLocalPlayer(), 0, 0, 20, "测试Frame按钮 点击了！");
        });
        //
        SyncUtil.onSyncData("gui_test", p => {
            DisplayTimedTextToPlayer(GetLocalPlayer(), 0, 0, 20, '接收到数据同步！触发的玩家为:' + GetPlayerName(p));
        });
        AsyncUtil.run(() => {
            new Gui();
        });
    ...
    archiveTest() {
        ...
        if (av) {
            ...
            DisplayTimedTextToPlayer(GetLocalPlayer(), 0, 0, 60, '从存档读对象表:' + ArchiveUtil.get(Player(0), "我存个对象").套娃.a);
        }
        else {
            ArchiveUtil.set(Player(0), key, "哈哈我使劲存啊，把存档占满，不用白不用！！！！，布尔值可存0跟1以节省空间。我这里其实可以存超过64个长度的字符串啊 最大可存1.9W个字符");
            ArchiveUtil.set(Player(0), "我存个对象", { "存个整数": 1, "存个布尔": true, "存个字符": "a", "套娃": { "a": "我是对象里的对象的套娃a数据" } });
        }
    }
    httpJsonTest() {
        se.playerChat("tj", () => {
