...
export default class SolarActorState {
    ...
    initActor() {
        ...
        SyncUtil.onSyncObjData("_sl_:a:onClick", (triggerPlayer, data) => {
            let actor = Actor.allActors[data.i];
            if (actor == null) {
            ...
            actor.get("onClick")?.(actor, data.b, triggerPlayer, actor.actorType);
        });
        SyncUtil.onSyncObjData("_sl_:at:onClick", (triggerPlayer, data) => {
            ...
            actorType.onClick?.(null, data.b, triggerPlayer, actorType);
        });
    }
    /**
     * 初始化演员 太阳伤害系统 事件
