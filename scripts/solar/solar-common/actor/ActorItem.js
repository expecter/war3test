...
/**
 * 注意物品不应该被单位A死 否则没有事件回调去销毁物品对应的关联数据 导致模版泄漏
 *
 * 优先保证写法兼容ActorBuff 这样吞噬后才有效果（既不要过多使用此子类中的具体功能 比如使用次数）
 *
 */
export default class ActorItem extends ActorAbility {
    static allActorItems = {};
    itemId;
    item;
    constructor(actorTypeId, x = 0, y = 0) {
        super(actorTypeId);
        if (this._actorType == null) {
            return;
        }
        ActorItem.allActorItems[this.uuid] = this;
        //
        let templateType = this.getTemplateType();
        if (templateType == null || _g_objs.item[templateType] == null || !ObjectTemplateUtil.hasTemplate(templateType)) {
            templateType = "主动物品";
        }
        this.itemId = ObjectTemplateUtil.borrowTemplate(templateType, this.templateCacheKey, this.uuid);
    ...
    _sl_rawset(key, value) {
        ...
        switch (key) {
            ...
            case "name":
                ...
                if (isEmbedJapi) {
                    //设置名字
                    EXSetItemDataString(this.itemId, 2, value);
                }
                break;
            case "describe":
                EXSetItemDataString(this.itemId, 3, value);
                if (isEmbedJapi) {
                    EXSetItemDataString(this.itemId, 5, value);
                }
                break;
            case "hide":
            ...
            case "model":
                //内置的设置单位可以 设置物品的模型
                if (SetUnitModel) {
                    SetUnitModel(this.item, value);
                }
                else if (DzItemSetModel) {
                    DzItemSetModel(this.item, value);
                }
                DzItemSetPortrait?.(this.item, value);
                break;
            case "modelScale":
                EXSetItemSize?.(this.item, value);
                break;
            case "colorR":
                let colorIntByRGB = ColorUtil.getColorIntByRGB(value || 255, this.get("colorG", 255), this.get("colorB", 255));
                EXSetItemColor?.(this.item, colorIntByRGB);
                break;
            case "colorG":
                let colorIntByRGB2 = ColorUtil.getColorIntByRGB(this.get("colorR", 255), value || 255, this.get("colorB", 255));
                EXSetItemColor?.(this.item, colorIntByRGB2);
                break;
            case "colorB":
                let colorIntByRGB3 = ColorUtil.getColorIntByRGB(this.get("colorR", 255), this.get("colorG", 255), value || 255);
                EXSetItemColor?.(this.item, colorIntByRGB3);
                break;
            case "disable":
                if (this.get("disable")) {
     ...
     */
    getPawnGold() {
        if (this.get("goldCost") != null && this.get("goldCost") > 0) {
            let val = Math.floor(this.get("goldCost") * PawnItemRate);
            if (val > 0) {
                if (this.get("stackMax", 0) > 0) {
                    val *= this.getUses();
                }
                //某些物品初始购买就是多个使用次数 这里要根据使用次数从新算下价格
                if (this.actorType.uses && this.actorType.uses > 1) {
                    val = Math.floor(val / this.actorType.uses);
                }
            }
            return val;
        }
     ...
     */
    getPawnLumber() {
        if (this.get("lumberCost") != null && this.get("lumberCost") > 0) {
            let val = Math.floor(this.get("lumberCost") * PawnItemRate);
            if (val > 0) {
                if (this.get("stackMax", 0) > 0) {
                    val *= this.getUses();
                }
    ...
    update() {
        ...
        if (this.getAbility() != null) {
            ObjectTemplateUtil.cleanAbility(this.getAbility());
        }
        super.update();
        //
        if (this.unit != null) {
        ...
        BaseUtil.runLater(0.01, () => {
            ...
            for (let i = 0; i < 6; i++) {
                if (UnitItemInSlot(this.unit, i) == this.item) {
                    ...
                    if (this.get("droppable") == false) {
                        SetItemDroppable(this.item, true);
    ...
    action(x, y, targetUnit) {
        ...
        if (this.get("destroyOnNoUses") == true || this.actorType.uses || this.actorType.stackMax || GetItemCharges(this.item) > 1) {
            //同步物品使用次数 到自定义变量uses
    ...
    destroy() {
        ...
        deleteKey(ActorItem.allActorItems, this.uuid);
    }
}
