...
export default class ActorTypeBuildByEffectUtil {
    ...
    static tempSolarBuildEffect = null;
     ...
     */
    static canBuildCheck = (actor, x, y) => {
     ...
     * @param onBuild
     */
    static warpUnit2BuildAbility(unitIdOrActorId, onLocalClickCheck, onBuild) {
     ...
     * @param onBuild
     */
    static addBuildUnitType(actorTypeId, unitId, onLocalClickCheck, onBuildFinish) {
        if (!isEmbedJapi) {
            print("无内置不支持此模拟建造方法!");
            return;
        }
    ...
    static setBuilds2unit(unit, builds) {
        ...
        let tongMoListStr = ObjectDataUtil.getAbilityDataString(spellBookIdStr, "DataA");
        // print("tongMoListStr="+tostring(tongMoListStr))
        let tongMoIds = tongMoListStr.split(",");
        for (let i = 0; i < builds.length; i++) {
            ...
            //显示图标
            SetUnitAbilityButtonShow(unit, FourCC(tongMoIds[i]), true);
            //更新图标
            actorAbility.update();
        }
        //隐藏图标
        for (let i = builds.length; i < tongMoIds.length; i++) {
            SetUnitAbilityButtonShow(unit, FourCC(tongMoIds[i]), false);
        }
    }
    /**
     * @deprecated see ActorTypeBuildUtil.setBuilds2unit
     ...
     * @param createDefault
     */
    static getBuildsMenuAbilityFromunit(unit, createDefault = true) {
