...
export default class SolarActorAttributeState {
     ...
     */
    static itemAttributeHandlers = [];
    static buffAttributeHandlers = [];
    ...
    static refreshActorAttributes2UnitSolarAttribute(unitHandle) {
        //死亡单位不刷新
        if (!UnitStateUtil.isAlive(unitHandle)) {
            return;
        }
        let attributes = SolarActorAttributeState.getUnitAllActorAttributes(unitHandle);
        let oldAttr = DataBase.getUnitSolarData(unitHandle, false)?._SL_totalActorsSolarAttribute;
