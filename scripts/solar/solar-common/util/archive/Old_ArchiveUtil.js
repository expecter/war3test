...
export default class Old_ArchiveUtil {
     ...
     * */
    static verifyPlayerMapLv = false;
    /**
     * 最大存档key 长度
     */
    static ArchiveKeyCountMax = 150 - 10;
    static salt = "s_z_b_s_q_j";
    //11平台最大8位
    static baseArchiveKeyPrefix = "SA_";
     ...
     */
    static get(whichPlayer, key, limitVal = 0, limitKey = Old_ArchiveUtil.defaultLimitKey) {
     ...
     */
    static set(whichPlayer, key, value, isSaveAll = true) {
        Old_ArchiveUtil._init0();
        let data = Old_ArchiveUtil.playerDatas["P" + GetPlayerId(whichPlayer)];
        if (!data) {
            data = {};
    ...
    static clearAllArchive(whichPlayer) {
        ...
        for (let i = 0; i < Old_ArchiveUtil.ArchiveKeyCountMax; i++) {
            let key = Old_ArchiveUtil.baseArchiveKeyPrefix + i;
    ...
    static saveAll(whichPlayer) {
        ...
        //清空后一个存档栏位 以标记结束
        let key = Old_ArchiveUtil.baseArchiveKeyPrefix + keyLength;
        DzAPI_Map_FlushStoredMission(whichPlayer, key);
