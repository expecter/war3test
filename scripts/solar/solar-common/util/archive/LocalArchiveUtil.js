...
export default class LocalArchiveUtil {
     ...
     */
    static get(whichPlayer, key, mapLevelLimit = 0) {
     ...
     * @param saveAll
     */
    static set(whichPlayer, key, value, saveAll = false) {
        LocalArchiveUtil._init0();
        let data = LocalArchiveUtil.playerDatas["P" + GetPlayerId(whichPlayer)];
        if (!data) {
            data = {};
     ...
     * 清空所有存档
     */
    static clearAllArchive(whichPlayer = GetLocalPlayer()) {
     ...
     * @param whichPlayer
     */
    static saveAll(whichPlayer = GetLocalPlayer()) {
        if (whichPlayer != GetLocalPlayer()) {
            return;
        }
        let data = LocalArchiveUtil.playerDatas["P" + GetPlayerId(whichPlayer)];
