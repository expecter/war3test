...
export default class ArchiveUtil {
     ...
     */
    static encrypt = true;
    static verifyPlayerName = !isDebug;
    /**
     * 最大存档key 长度
     */
    static ArchiveKeyCountMax = 120;
    static salt = "s_z_b_s_q_j";
    //11平台最大8位
    static defaultLimitKey = null;
    static playerDatas = {};
    static _sl_playerCurrentArchiveKeyMax = {};
    static _init_time = -1;
    /** solar Archive v2 addon */
    static playerDataBuckets = {};
    /** 发布前确定 不然更改此选项后会导致存档错乱 */
     ...
     */
    static get(whichPlayer, key, limitVal = 0, limitKey = ArchiveUtil.defaultLimitKey) {
        ArchiveUtil._init0();
        if (limitVal > 0) {
     ...
     * @param value
     */
    static updateMaxNumber(whichPlayer, key, value) {
        let oldVal = ArchiveUtil.get(whichPlayer, key) || 0;
        if (value == null) {
            return oldVal;
        }
        if (value > oldVal) {
            ArchiveUtil.set(whichPlayer, key, value);
            return value;
        }
        return oldVal;
    }
     ...
     * @param addValue
     */
    static addNumber(whichPlayer, key, addValue) {
        let oldVal = ArchiveUtil.get(whichPlayer, key) || 0;
        let newVal = oldVal + addValue;
        ArchiveUtil.set(whichPlayer, key, newVal);
        return newVal;
    }
    /**
     * 保存玩家的一个存档
     * @param whichPlayer
     * @param key
     * @param value 传null则删除此key与数据
     * @param isSave  立即上传！是否立即上传到服务器存档(频繁设置值时可以先填false 最后再统一上传 节省网络性能)
     */
    static set(whichPlayer, key, value, isSave = true) {
     ...
     * @deprecated
     */
    static saveAll(whichPlayer, ignoreWarning = false) {
        if (ignoreWarning == false) {
            log.errorWithTraceBack("不推荐的用法!一次性保存所有存档可能导致崩溃丢档!");
        }
        //
        let playerDataBucket = ArchiveUtil.playerDataBuckets["P" + GetPlayerId(whichPlayer)];
        for (let bucketId in playerDataBucket) {
            ArchiveUtil._sl_saveBucket(whichPlayer, bucketId, playerDataBucket[bucketId]);
        }
    }
    /**
     *获取所有存档
     */
    static getAllArchive(whichPlayer) {
        ArchiveUtil._init0();
     ...
     * @param whichPlayer
     */
    static printAllArchive(whichPlayer) {
        ArchiveUtil._init0();
        let data = ArchiveUtil.playerDatas["P" + GetPlayerId(whichPlayer)];
        if (!data) {
            return;
     ...
     * 清空所有存档
     */
    static clearAllArchive(whichPlayer) {
        ArchiveUtil._init0();
     ...
     * 获取玩家当前使用的存档栏位
     */
    static getCurrentArchiveKeyMax(whichPlayer) {
        ArchiveUtil._init0();
     ...
     * 获取玩家当前使用的存档栏位 提示信息
     */
    static getCurrentArchiveKeyMaxInfo(whichPlayer) {
    ...
    static _sl_saveBucket(whichPlayer, bucketId, bucket) {
        let dataJsonStr = JSON.stringify(bucket);
        ...
        let useBatch = !isDebug; //正式环境用批量保存 本地用基础保存才能保存到本地存档文件。
        // let useBatch: boolean = false;//先统一单个保存
        if (useBatch) {
    ...
    static _init0() {
        ...
        se.onPlayerChat("-sl-ia", (e) => {
            ...
            ArchiveUtil.printAllArchive(e.triggerPlayer);
        });
        se.onPlayerChat("-清空存档", (e) => {
            try {
                for (let allArchiveKey of PlatUtil.allArchiveKeys) {
    ...
    static _init0_onePlayer(tempPlayer) {
        let data = {};
        ...
        for (let bIndex = 0; bIndex < ArchiveUtil.bucketMax; bIndex++) {
            ...
            let bucketDataJsonStr = "";
            for (let i = 0; i < ArchiveUtil.ArchiveKeyCountMax; i++) {
                let key = bucketId + "_" + i;
                let serverValue = DzAPI_Map_GetServerValue(tempPlayer, key);
                if (serverValue && serverValue.length > 0) {
            ...
            try {
                if (ArchiveUtil.encrypt) {
                    ...
                    if (bucketDataJsonStr == null) { //存档解密失败 说明玩家改存档了 (也可能是这个bucket坏了 比如刚存的时候就退出游戏 没存完整)
                        print("存档Json格式解析出错!");
                        DisplayTimedTextToPlayer(GetLocalPlayer(), 0, 0, 60, "|cffff0000玩家" + (pId + 1) + ":" + GetPlayerName(tempPlayer) + ' 的存档格式解析出错！');
