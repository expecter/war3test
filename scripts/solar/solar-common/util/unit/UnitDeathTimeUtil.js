...
export default class UnitDeathTimeUtil {
    static _sl_deathTimeData = {};
    /**
     * 设置死亡时间 (毫秒单位)
     * @param deathUnit
     * @param deathTime
     */
    static setDeathTime(deathUnit, deathTime = time) {
     ...
     * @param deathTime
     */
    static clearDeathTime(deathUnit) {
     ...
     */
    static isTimeOfDeathExceeded(deathUnit, timeSlot = handleReuseMinTime) {
        if (UnitAlive(deathUnit)) {
            //还是活着的单位
            return false;
        }
