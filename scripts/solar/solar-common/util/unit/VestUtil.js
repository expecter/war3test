...
export default class VestUtil {
     ...
     * @constructor
     */
    static SunCreateVestByUnit(u, x = GetUnitX(u), y = GetUnitY(u), model = "", moveType = 0, timer = 2, player) {
     ...
     */
    static createVestByUnit(u, x = GetUnitX(u), y = GetUnitY(u), model = "", moveType = 0, timer = 2, player) {
        if (player) {
    ...
    static createVest(player, x, y, model = "", moveType = 0, timer = 2) {
        let mj = CreateUnit(player, FourCC('hrdh'), x, y, 0);
    ...
    static _sl_createPhantomUnitTrigger = null;
    /**
     ...
     * @param owner
     */
    static createPhantomUnit(unit, lifeTime = 15, damageP = 1, underDamageP = 2, owner) {
        if (!IsHandle(unit)) {
            return;
        }
        //初始化属性
