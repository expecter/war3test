...
export default class UnitUtil {
     ...
     */
    static calculateDamageByPropertySet(unitHandle, dmgPropertySet) {
        let damage = 0;
     ...
     * @constructor
     */
    static GetInventoryIndexOfItemType(unitHandle, id) {
        let idint = FourCC(id);
        for (let i = 0; i < 6; i++) {
     ...
     * @constructor
     */
    static GetInventoryOfItemType(unitHandle, idint) {
        for (let i = 0; i < 6; i++) {
            let indexItem = UnitItemInSlot(unitHandle, i);
            if (IsHandle(indexItem) && GetItemTypeId(indexItem) == idint) {
                return indexItem;
     ...
     * @param addNum
     */
    static addHeroProperty(unit, key, addNum) {
        if (!unit.isHero()) {
            return;
        }
        //力量
     ...
     * @param addRate
     */
    static addHeroPropertyByRate(unit, key, addRate) {
        if (!unit.isHero()) {
            return;
        }
        let addNum = 0;
     ...
     */
    static transfer2Rect(unitHandle, rect, faceNearestEnemy = true, effectPath = "Abilities\\Spells\\Human\\MassTeleport\\MassTeleportCaster.mdx") {
     ...
     * @param effectPath 目标点特效
     */
    static transfer(unitHandle, x, y, faceNearestEnemy = true, effectPath = "Abilities\\Spells\\Human\\MassTeleport\\MassTeleportCaster.mdx") {
     ...
     * 面向最近的敌人
     */
    static faceNearestEnemy(unitHandle, maxRange = 2000) {
     ...
     * 返回最后一个创建的演员单位
     */
    static createUnit(player, unitTypeId, x, y, face = 0, count = 1) {
        let unit = null;
     ...
     */
    static isBack(attackedUnit, attacker) {
        let targetLoc = GetUnitLoc(attackedUnit);
        let triggerLoc = GetUnitLoc(attacker);
        let angle = AngleBetweenPoints(triggerLoc, targetLoc);
        let facing = GetUnitFacing(attackedUnit);
     ...
     *设置附加魔法值
     */
    static setExtraMana(unitHandle, val, key = "base") {
    ...
    static getExtraMana(unitHandle, key) {
        return UnitUtil.getExtraValue(unitHandle, '_SLExt_Mana', key);
    }
    /**
     *设置附加生命值
     */
    static setExtraHp(unitHandle, val, key = "base") {
    ...
    static getExtraHp(unitHandle, key) {
        return UnitUtil.getExtraValue(unitHandle, '_SLExt_Hp', key);
    }
    /**
     *设置附加的攻击间隔 负数为减少攻击间隔
     */
    static setExtraDamageCool(unitHandle, val, key = "base") {
     ...
     *设置附加的攻击范围
     */
    static setExtraDamageRange(unitHandle, val, key = "base") {
     ...
     *设置附加的移动速度
     */
    static setExtraMoveSpeed(unitHandle, val, key = "base") {
     ...
     *设置攻击速度 0.5 = 50%
     */
    static setExtraAttackSpd(unitHandle, val, key = "base") {
        //AIs2 = 能提高进攻速度的物品
     ...
     * @param unitHandle
     */
    static getExtraAttackSpd(unitHandle) {
        return UnitUtil.getExtraValue(unitHandle, '_SLExt_AttackSpd');
    }
    /**
     *设置附加攻击 (即绿字攻击力)
     */
    static setExtraAttack(unitHandle, val, key = "base") {
        //AItg = 攻击之爪+1
     ...
     * @param unitHandle
     */
    static getExtraAttack(unitHandle) {
        return UnitUtil.getExtraValue(unitHandle, '_SLExt_Attack');
    }
    /**
     *设置附加护甲 (即绿字护甲)
     */
    static setExtraDef(unitHandle, val, key = "base") {
        //AId1 = 能提高护甲的物品(1)
     ...
     * @param unitHandle
     */
    static getExtraDef(unitHandle, key) {
     ...
     * @param key
     */
    static setExtraStrAgiInt(unitHandle, key, str, agi, int) {
     ...
     *设置附加力量 (即绿字力量)
     */
    static setExtraStr(unitHandle, val, key = "base") {
     ...
     * @param unitHandle
     */
    static getExtraStr(unitHandle, key) {
        if (key) {
     ...
     *设置附加敏捷 (即绿字敏捷)
     */
    static setExtraAgi(unitHandle, val, key = "base") {
     ...
     * @param unitHandle
     */
    static getExtraAgi(unitHandle, key) {
        if (key) {
     ...
     *设置附加智力 (即绿字智力)
     */
    static setExtraInt(unitHandle, val, key = "base") {
        let str_val = UnitUtil.getExtraStr(unitHandle);
        let agi_val = UnitUtil.getExtraAgi(unitHandle);
     ...
     * @param unitHandle
     */
    static getExtraInt(unitHandle, key) {
        if (key) {
     ...
     * @param key
     */
    static extraValueSum(unitHandle, type, val, key = "base") {
        let solarData = DataBase.getUnitSolarData(unitHandle);
        let extraData = solarData[type];
        if (val) {
            if (!extraData) {
                extraData = {};
     ...
     */
    static getExtraValue(unitHandle, type, key) {
     ...
     * @param value
     */
    static refreshUnitAbilityData(unitHandle, abilcode, data_type, value) {
     ...
     * @param int
     */
    static refreshUnitAamkData(unitHandle, str, agi, int) {
        ...
        if (isBigAttributeMode) {
            //10e
            EXSetAbilityDataReal(ability, 1, ABILITY_DATA_DATA_A, MathUtil.clamp(agi, -BaseAttributeMax, BaseAttributeMax));
            EXSetAbilityDataReal(ability, 1, ABILITY_DATA_DATA_B, MathUtil.clamp(int, -BaseAttributeMax, BaseAttributeMax));
            EXSetAbilityDataReal(ability, 1, ABILITY_DATA_DATA_C, MathUtil.clamp(str, -BaseAttributeMax, BaseAttributeMax));
            //力量奖励生命值
