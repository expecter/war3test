...
export default class SelectUtil {
     ...
     * @param clearCache
     */
    static getAnHero(playerIndex, clearCache = false) {
     ...
     * @param callBack
     */
    static forUserPlayerHero(callBack) {
        for (let i = 0; i < bj_MAX_PLAYER_SLOTS; i++) {
            ...
            if (GetPlayerController(tempPlayer) == MAP_CONTROL_USER && GetPlayerSlotState(tempPlayer) == PLAYER_SLOT_STATE_PLAYING) {
                let anHero = SelectUtil.getAnHero(GetPlayerId(tempPlayer));
                if (IsHandle(anHero)) {
                    callBack(anHero, tempPlayer);
                }
            }
        }
    }
     ...
     * @param canAttack
     */
    static getAnEnemy(playerIndex = 0, canAttack = true) {
     ...
     * @param playerIndex
     */
    static getAllEnemys(playerIndex = 0) {
        let enemyUnits = [];
     ...
     * @param playerIndex
     */
    static getAllEnemysLength(playerIndex = 0) {
        let allUnits = GameCenter.getAllUnits();
        let player = Player(playerIndex);
        let count = 0;
        for (let unit of allUnits) {
     ...
     */
    static forAllyUnitsInRange(center, radius, callback, x = GetUnitX(center), y = GetUnitY(center)) {
        let group = GroupUtil.groupObjectPool.borrowObject();
     ...
     */
    static forEnemyUnitsInRect(center, rect, callback) {
        let group = GroupUtil.groupObjectPool.borrowObject();
     ...
     * 遍历 单位 (可手动指定范围中心点x,y)周围的敌方单位
     */
    static forEnemyUnitsInRange(center, radius, callback, x = GetUnitX(center), y = GetUnitY(center)) {
        let group = GroupUtil.groupObjectPool.borrowObject();
        GroupEnumUnitsInRange(group, x, y, radius, null);
        let myPlayer = GetOwningPlayer(center);
        for (let i = 0; i <= 1000000; i++) {
     ...
     */
    static getEnemyUnitsInRangeOrderByDistance(center, radius, x = GetUnitX(center), y = GetUnitY(center), notInvulnerable = true) {
     ...
     */
    static getEnemyUnitsInRange(center, radius, x = GetUnitX(center), y = GetUnitY(center), notInvulnerable = true) {
        let group = GroupUtil.groupObjectPool.borrowObject();
        GroupEnumUnitsInRange(group, x, y, radius, null);
        let units = [];
        let myPlayer = GetOwningPlayer(center);
     ...
     * 可异步使用
     */
    static getUnitsInRange(x, y, radius) {
        GroupClear(_tempGroup);
        GroupEnumUnitsInRange(tempGroup, x, y, radius, null);
        let units = GroupUtil.toArray(tempGroup);
        return units;
    }
    /**
     * 遍历中心点x,y周围的所有单位
     */
    static forUnitsInRange(x, y, radius, callback) {
        let group = GroupUtil.groupObjectPool.borrowObject();
        GroupEnumUnitsInRange(group, x, y, radius, null);
     ...
     * @param whoEnemy  发起攻击的单位
     * @param lineX  线的中心点x
     * @param lineY  线的中心点y
     * @param lineLen  线的长度
     * @param lineFaceAngle  线的角度
     * @param lineWidth  线的宽度
     * @param callback  回调
     */
    static forEnemyUnitsInLine(whoEnemy, lineX, lineY, lineLen, lineFaceAngle, lineWidth, callback) {
     ...
     */
    static getEnemyUnitsInLine(whoEnemy, lineX, lineY, lineLen, lineFaceAngle, lineWidth) {
        let unitsInLine = SelectUtil.getUnitsInLine(lineX, lineY, lineLen, lineFaceAngle, lineWidth);
        if (unitsInLine == null || unitsInLine.length == 0) {
            return;
        }
     ...
     * @param callback
     */
    static forUnitsInLine(lineX, lineY, lineLen, lineFaceAngle, lineWidth, callback) {
        let unitsInLine = SelectUtil.getUnitsInLine(lineX, lineY, lineLen, lineFaceAngle, lineWidth);
        if (unitsInLine == null || unitsInLine.length == 0) {
            return;
        }
     ...
     * @param lineWidth
     */
    static getUnitsInLine(lineX, lineY, lineLen, lineFaceAngle, lineWidth) {
     ...
     * @param unitType
     */
    static forAllAliveUnits(callback) {
        let allUnits = GameCenter.getAllUnits();
     ...
     * @param unitType
     */
    static forAllUnits(callback, unitType) {
        let allUnits = GameCenter.getAllUnits();
     ...
     * @param onlyAlive
     */
    static getPlayerUnits(playerIndex, unitType, onlyAlive) {
        let resultUnits = [];
        let group = GroupUtil.groupObjectPool.borrowObject();
        GroupEnumUnitsOfPlayer(group, Player(playerIndex), null);
        if (unitType) {
     ...
     */
    static forPlayerUnits(callback, playerIndex, unitType) {
        let group = GroupUtil.groupObjectPool.borrowObject();
        GroupEnumUnitsOfPlayer(group, Player(playerIndex), null);
        if (unitType) { //如果指定类型存在 则按指定类型遍历
            let unitTypeNum = FourCC(unitType);
     ...
     * @param unitType 遍历指定单位类型 不传值则为所有类型
     */
    static forPlayerAliveUnits(callback, playerIndex) {
        let group = GroupUtil.groupObjectPool.borrowObject();
        GroupEnumUnitsOfPlayer(group, Player(playerIndex), null);
        for (let i = 0; i <= 1000000; i++) {
            let unitHandle = FirstOfGroup(group);
     ...
     * @param whoseEnemy
     */
    static forAllEnemyUnits(callback, whoseEnemy = Player(0)) {
        let group = GroupUtil.groupObjectPool.borrowObject();
    ...
    static getFarthestEnemyInRange(whichUnit, range) {
        let units = SelectUtil.getEnemyUnitsInRange(whichUnit, range);
        let returnUnitHandle = null;
        let tempDistance = -1;
        for (let unitHandle of units) {
            let distance = MathUtil.distanceBetweenPoints(GetUnitX(whichUnit), GetUnitY(whichUnit), GetUnitX(unitHandle), GetUnitY(unitHandle));
     ...
     * （可以模拟血条类似情况下使用）
     */
    static forLocalCameraUnits(inCameraUnitsCallback, outCameraUnitsCallback = null, frustumTop = 0.56, frustumBottom = 0.13, frustumLeft = 0, frustumRight = 0.8) {
        if (!isEmbedJapi) {
