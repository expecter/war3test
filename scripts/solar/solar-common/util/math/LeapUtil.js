...
export default class LeapUtil {
     ...
     */
    static leap(damage_src_handle, angle, range, time, damage = 0, dmgType = DamageType.s_物理, effectPath, startX = GetUnitX(damage_src_handle), startY = GetUnitY(damage_src_handle), onHitUnit) {
     ...
     * @param onDone 完成时的回调
     */
    static infiniteLeap(unit, angle, speed = 1000, panCamera = false, onDone) {
        ...
        BaseUtil.onTimer(0.04, (c) => {
            ...
            //最多12秒
            if (RectUtil.isInPlayableArea(x, y) && c < 300 && !IsTerrainPathable(x, y, PATHING_TYPE_FLYABILITY)) {
