...
export default class VectorUtil {
    ...
    static getVector(data) {
        ...
        if (LangUtil.isString(data) && data.endsWith("]")) {
            let solarLineInfo = data;
            ...
            let solarLineIndex = tonumber(solarLineInfo.substring(indexOf + 1, solarLineInfo.length - 1));
            try {
                let _require = require;
                let ____exports = _require(solarLineName);
                let [key, lineVals] = next(____exports);
                let vectors = lineVals;
