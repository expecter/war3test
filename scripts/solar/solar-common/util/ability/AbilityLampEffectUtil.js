...
export default class AbilityLampEffectUtil {
     ...
     * @param unit
     */
    static _sl_unitObjIdLampEffectInfo = {};
    static _sl_lampEffectFrames = {};
    /**
     * 为玩家下的所有单位此时拥有此技能的 单位 添加按钮跑马灯转圈特效
     * @param player
     * @param abilityId
     */
    static addAbilityLampEffectForPlayer(abilityId, player) {
     ...
     * @param abilityId
     */
    static removeAbilityLampEffectForPlayer(abilityId, player) {
        let playerUnits = SelectUtil.getPlayerUnits(GetPlayerId(player));
        for (let playerUnit of playerUnits) {
            if (GetUnitAbilityLevel(playerUnit, abilityId) > 0) {
     ...
     */
    static addUnitObjIdLampEffect(abilityIdOrUnitId, unit) {
     ...
     */
    static removeUnitObjIdLampEffect(abilityIdOrUnitId, unit) {
        AbilityLampEffectUtil._sl_init_lampEffect_system();
        let abilityIdStrs = AbilityLampEffectUtil._sl_unitObjIdLampEffectInfo[GetHandleId(unit)];
        if (abilityIdStrs == null) {
            return;
    ...
    static _sl_init_lampEffect_system() {
    ...
    static refreshUnitAbilityLampEffect(selectUnit) {
        //隐藏全部
