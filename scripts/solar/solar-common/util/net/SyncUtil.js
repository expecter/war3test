...
export default class SyncUtil {
     ...
     * 只能在同步里调用此方法
     */
    static onSyncData(key, actionFunc, isServer = false) {
        if (isAsync) {
     ...
     * @param actionFunc
     */
    static onSyncObjData(key, actionFunc) {
        if (isAsync) {
            log.errorWithTraceBack("不能在异步中调用此方法！请在同步方法中执行！");
            return;
        }
     ...
     * @param actionFunc
     */
    static onSyncBigData(key, actionFunc) {
        if (isAsync) {
            log.errorWithTraceBack("不能在异步中调用此方法！请在同步方法中执行！");
            return;
        }
        ...
        TriggerAddAction(trigger, () => {
            ...
            let dataStr = DzGetTriggerSyncData();
            // @ts-ignore
            let dataPacket = JSON.parse(dataStr);
            //组装数据
            let keyDataPackets = SyncUtil.bigDataBuffer[key];
            if (!keyDataPackets) {
                keyDataPackets = [];
