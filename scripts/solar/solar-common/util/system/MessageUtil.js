...
export default class MessageUtil {
    ...
    static windowEventCallBacks = [];
    static _sl_isInitialized = false;
    /** 右键拖拽物品 */
    static _sl_lastReleaseItem = null;
    /**
     *
     * @param type
     * @param callBack 返回true 表示忽略此次点击
     */
    static addHookEventCallBack(type, callBack) {
     ...
     * @param callBack 返回true 表示忽略此次点击
     */
    static addWindowEventCallBack(callBack) {
        MessageUtil.windowEventCallBacks.push(callBack);
    }
    static _sl_onHookMsg(msg) {
        let type = msg.type;
        if (type == null) {
            return true;
        }
        if (type == "key_down") {
            ...
            if ((1096114805 == msg.ability || 1095262837 == msg.ability) && msg.code == 1) {
                ...
                //
                let actor = ActorAbilityUtil.getActorAbilityByBaseId(id2string(msg.ability), selection());
                if (actor != null) {
    ...
    static _sl_init_message_hook() {
        ...
        //
        const message = require('jass.message');
        ...
        if (isEmbedJapi) {
            let oldWindowEventCallBack = _G["WindowEventCallBack"];
