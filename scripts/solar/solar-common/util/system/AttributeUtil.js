...
export default class AttributeUtil {
     ...
     * @param attribute
     */
    static getAttributeInfo(attribute) {
        let info = "";
        let keys = [];
        //收集key
     ...
     */
    static getUnitAttribute(unitHandle, createDefault = false) {
     ...
     * @param createDefault 是否在属性对象对象为空时默认创建一个属性对象对象
     */
    static getUnitTypeAttribute(unitTypeId, createDefault = false) {
     ...
     * @param addAttribute
     */
    static addUnitAttribute(unitHandle, addAttribute) {
        if (!addAttribute) {
            return;
        }
        ...
        for (let key in addAttribute) {
            if (typeof addAttribute[key] == "number") {
                baseAttribute[key] = (baseAttribute[key] ?? 0) + addAttribute[key];
            }
     ...
     * @param createDefault 是否在属性对象对象为空时默认创建一个属性对象对象
     */
    static getItemAttribute(itemHandle, createDefault = false) {
     ...
     * @param createDefault 是否在属性对象对象为空时默认创建一个属性对象对象
     */
    static getItemTypeAttribute(itemTypeId, createDefault = false) {
     ...
     */
    static setItemTypeAttribute(itemTypeId, attribute, allowCover = false) {
     ...
     * @param createDefault
     */
    static getPlayerAttribute(playerHandle, createDefault = false) {
     ...
     * @param addAttribute
     */
    static addPlayerAttribute(playerHandle, addAttribute) {
        if (!addAttribute) {
            return;
        }
        ...
        for (let key in addAttribute) {
            if (typeof addAttribute[key] == "number") {
                basePlayerAttribute[key] = (basePlayerAttribute[key] ?? 0) + addAttribute[key];
            }
     ...
     */
    static setPlayerAttribute(playerHandle, attribute, allowCover = false) {
     ...
     */
    static setUnitAttribute(unitHandle, attribute, allowCover = false) {
     ...
     * @param allowCover 是否允许覆盖 若为false时 则不能为一个已有属性的单位重新设置全部属性  通常为false 除非你确定你自己在干嘛
     */
    static setUnitTypeAttribute(unitTypeId, attribute, allowCover = false) {
     ...
     * @param allowCover
     */
    static setItemAttribute(itemHandle, attribute, allowCover = false) {
     ...
     * 循环地图上 所有有属性的单位
     */
    static forAllUnitsAttribute(callback) {
     ...
     * 循环所有玩家属性
     */
    static forAllPlayerAttribute(callback, createDefault = false) {
     ...
     * @param attributes
     */
    static sumAttributes(attributes) {
        let result = {};
        if (attributes == null) {
            return result;
        }
     ...
     */
    static add(attribute, _attribute) {
        if (!_attribute) {
            return;
        }
        for (let key in _attribute) {
            if (typeof _attribute[key] == "number") {
                attribute[key] = (attribute[key] ?? 0) + _attribute[key];
            }
        }
    }
    /**
     ...
     *  attribute1 - attribute2
     */
    static subtract(attribute, _attribute) {
        if (!_attribute) {
            return;
        }
        for (let key in _attribute) {
     ...
     *  attribute * scale
     */
    static multiply(attribute, scale, store) {
        if (!attribute) {
            return;
        }
        if (scale == 0) {
        ...
        for (let key in attribute) {
            if (typeof attribute[key] == "number") {
                store[key] = (attribute[key] ?? 0) * scale;
            }
        }
        return store;
    }
     ...
     *  属性值是否相等
     */
    static isEquals(attribute, otherAttribute) {
        ...
        let keys1 = Object.keys(attribute);
        let keys2 = Object.keys(otherAttribute);
     ...
     * @param abilityIdStr
     */
    static getAbilityCDP(unitHandle, abilityIdStr) {
        let attribute = AttributeUtil.getUnitAttribute(unitHandle, false);
        return attribute?.["ability_cd_p_" + abilityIdStr] || 0;
    }
    /**
     * 设置技能cd 比例
     * @param unitHandle
     ...
     * @param val
     */
    static setAbilityCDP(unitHandle, abilityIdStr, val) {
     ...
     * @param val
     */
    static addAbilityCDP(unitHandle, abilityIdStr, val) {
        let attribute = AttributeUtil.getUnitAttribute(unitHandle, true);
        let key = "ability_cd_p_" + abilityIdStr;
        attribute[key] = (attribute[key] || 0) + val;
    }
    /**
     ...
     * @param abilityIdStr
     */
    static getAbilityCD(unitHandle, abilityIdStr) {
        let attribute = AttributeUtil.getUnitAttribute(unitHandle, false);
        return attribute?.["ability_cd_" + abilityIdStr] || 0;
    }
    /**
     ...
     * @param val
     */
    static setAbilityCD(unitHandle, abilityIdStr, val) {
        let attribute = AttributeUtil.getUnitAttribute(unitHandle, true);
     ...
     * @param val
     */
    static addAbilityCD(unitHandle, abilityIdStr, val) {
        let attribute = AttributeUtil.getUnitAttribute(unitHandle, true);
