...
export default class DoubleItemSlotUtil {
    ...
    static _sl_doubleItemSlotByAbility_CON() {
        let u = null;
        let trg = GetTriggeringTrigger();
        let hero = GetTriggerUnit();
        let trgSD = DataBase.getDataByHandle("trigger", trg);
        let ab = trgSD.Ability;
        let ItemSlotMax = trgSD.ItemSlotMax;
        let i = 0;
     ...
     * @param MaxItemSlot
     */
    static doubleItemSlotByAbility(hero, ab, MaxItemSlot, uid) {
        let heroSD = DataBase.getUnitSolarData(hero);
        let i = heroSD.MulItemSlotByAbility ?? 0;
        let trg = null;
        let u = null;
        //使用默认的 帮助单位
        let useDefaultHelpUnit = (uid == null);
        //
