...
export default class DialogUtil {
    static _sl_base_dialog_datas = {};
    /**
     * 返回的dialog 不要销毁了 以免后续其他地方使用 不生效
     * @param playerId
     * @private
     */
    static getBaseDialog(playerId) {
        if (isAsync) {
        ...
        if (baseDialogData == null) {
            ...
            TriggerAddAction(trigger, () => {
                ...
                dialogData.onClickButtonCallBack?.(buttonData._sl_index, buttonData._sl_text);
            });
        }
     ...
     * 返回的dialog 不要销毁了 以免后续其他地方使用 不生效
     */
    static show(playerId, title, onClickButtonCallBack, ...buttonTexts) {
     ...
     * 返回的dialog 不要销毁了 以免后续其他地方使用 不生效
     */
    static showPage(playerId, title, onClickButtonCallBack, pageIndex, pageSize, ...buttonTexts) {
        let baseDialogData = DialogUtil.getBaseDialog(playerId);
     ...
     * 返回的dialog 不要销毁了 以免后续其他地方使用 不生效
     */
    static showWithTimeLimit(playerId, title, lifeTime, onClickButtonCallBack, ...buttonTexts) {
        let baseDialogData = DialogUtil.getBaseDialog(playerId);
