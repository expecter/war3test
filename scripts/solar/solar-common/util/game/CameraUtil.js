...
export default class CameraUtil {
     ...
     * @param y
     */
    static getWordCoordinates(x = InputUtil.getMouseSceneX(), y = InputUtil.getMouseSceneY()) {
        if (isEmbedJapi) {
...
配合UI 可以画英雄称号 商店标题 等等
     ...
     * 屏幕y轴 下到上 0 ~ 0.6
     */
    static getScreenCoordinates(x, y, z = 150) {
        if (world_to_screen) {
     ...
     * 设置镜头高度
     */
    static setCameraHeight(height, whichPlayer) {
        if (whichPlayer) {
     ...
     * 平移镜头高度
     */
    static moveCamera2xy(x, y, whichPlayer, duration = 0) {
        if (whichPlayer) {
            if (GetLocalPlayer() == whichPlayer) {
     ...
     * @param duration
     */
    static moveCamera2unit(unit, whichPlayer = GetOwningPlayer(unit), duration = 0) {
        if (GetLocalPlayer() == whichPlayer) {
     ...
     */
    static setCameraBoundsByRect(r, mapMapPath) {
     ...
     * @param size
     */
    static setMapBlpRectByCameraBounds(fullMapPath = "war3mapMap.blp", size = 256) {
