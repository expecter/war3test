export default class DebugVmUtil {
     ...
     */
    static sethook(hook, mask) {
        debug.sethook(hook, mask, 0);
    }
    /**
     * 根据函数调用对战 获得相关的用户脚本文件加载 路径
     */
    static getUserScriptRequireModNameByStack() {
        let userScriptPaths = [];
        for (let i = 1; i < 200; i++) {
     ...
     * @param functionInfo
     */
    static getTsSrcLineName(functionInfo) {
        if (!functionInfo) {
            return "";
        }
     ...
     */
    static hookCRAndPrintInfo(writeFile = true, printMaxCount = 2, mask = "c") {
        let pt = {};
