...
let scale = 1600 * 1.25;
const smartSize = (size) => size < 1 && size > -1 ? size : size / scale;
export const setPixelScale = (newScale) => {
    scale = newScale * 1.25;
};
export const tooltipMap = {};
const defaultSync = false;
const setEventProp = (frame, 
// This typing is wrong, should be prop: K, value?: FrameProps[K]
event, val, oldValue, sync = false) => {
    if (sync && isAsync) {
...
const previousToParentPoint = (relative) => {
    switch (relative) {
        // Span
...
export default class AdapterUtil {
    static resolveRelative(frame, relative, relativePoint) {
        if (!relative) {
            if (frame != 0) {
     ...
     * setProp
     */
    static setProp(frame, prop, value, oldValue) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const val = value ?? frameDefaults[prop];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        switch (prop) {
            ...
            case "texture": {
                ...
                DzFrameSetTexture(frame, val2.texFile ?? frameDefaults.texture.texFile, val2.flag ?? frameDefaults.texture.flag);
                break;
            }
            case "model": {
                DzFrameSetModel(frame, val.modelFile ?? frameDefaults.model.modelFile, val.cameraIndex ?? frameDefaults.model.cameraIndex, 1);
                break;
            }
            case "alpha": {
            ...
            case "size": {
                DzFrameSetSize(frame, smartSize(val.width ?? frameDefaults.size.width), smartSize(val.height ?? frameDefaults.size.height));
                break;
            }
            case "stepSize": {
            ...
            case "frametip":
                ...
                break;
            case "font": {
                DzFrameSetFont(frame, val.fileName ?? frameDefaults.font.fileName, val.height ?? frameDefaults.font.height, val.flags ?? frameDefaults.font.flags);
                break;
            }
            case "minMaxValue": {
                DzFrameSetMinMaxValue(frame, val.min ?? frameDefaults.minMaxValue.min, val.max ?? frameDefaults.minMaxValue.max);
                break;
            }
            case "scale": {
            ...
            case "absPosition": {
                if (val != null) {
                    const positions = val === "clear" || "point" in val ? [val] : val;
                    for (const absPosition of positions) {
                        if (absPosition === "clear") {
                            DzFrameClearAllPoints(frame);
                        }
                        else {
                            DzFrameSetAbsolutePoint(frame, absPosition.point, smartSize(absPosition.x ?? 0), smartSize(absPosition.y ?? 0));
                        }
                    }
                }
                break;
    ...
    ;
    static setProp_position(frame, val) {
        if (val == null) {
            return;
        }
        const positions = val === "parent" || val === "clear" || "x" in val ? [val] : val;
        for (const position of positions) {
            ...
            if (relative) {
                DzFrameSetPoint(frame, position.point ?? 6, relative ?? DzFrameGetParent(frame), position.relativePoint ?? 6, smartSize(position.x ?? 0), smartSize(position.y ?? 0));
                return;
            }
            // We used `previous` and we're the first child
            const parentRelative = previousToParentPoint(position.relativePoint);
            if (parentRelative) {
                DzFrameSetPoint(frame, position.point, DzFrameGetParent(frame), parentRelative, smartSize(position.x ?? 0), smartSize(position.y ?? 0));
            }
        }
    }
}
