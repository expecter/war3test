...
export default class CreateFrameUtil {
    static nextFrameName() {
        frameDefaultsNameIndex++;
        return frameDefaults.name + "" + frameDefaultsNameIndex;
    }
    static createFrame(jsxType, parentFrame, props) {
        if (!parentFrame) {
            throw `expected parent frame for ${jsxType}`;
        }
        let { id, name, priority = frameDefaults.priority, inherits, isSimple, position, size, context = frameDefaults.context, ref, } = props;
        ...
        if (isSimple ?? jsxType === "simple-frame") {
            ...
            frame = DzCreateFrameByTagName(typeName, name, parentFrame, inherits ?? "", context);
        ...
        if (!position && !size) {
            ...
            DzFrameSetPoint(frame, 6, DzFrameGetParent(frame), 6, 0, 0);
        }
        else if (!size) {
