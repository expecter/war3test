...
export default class ItemAttributeState {
     ...
     * @param unitHandle
     */
    static getItemsAttributes(unitHandle) {
        //统计装备总计数值
        ...
        for (let i = 0; i < 6; i++) {
            ...
            //solarData 数据
            let itemAttribute = AttributeUtil.getItemAttribute(itemHandle);
            if (itemAttribute) {
     ...
     * @param unitHandle
     */
    static refreshItemAttributes2UnitSolarAttribute(unitHandle) {
        let itemAttributes = ItemAttributeState.getItemsAttributes(unitHandle);
        let unitOldItemSAttr = DataBase.getUnitSolarData(unitHandle, false)?._SL_totalItemsSolarAttribute;
        if (unitOldItemSAttr == null) { //如果之前没有旧物品属性 则退出后续逻辑  否则还需将之前的旧属性 移除
            if (itemAttributes == null || itemAttributes.length == 0) {
                return;
            }
        }
        let totalAttribute = AttributeUtil.sumAttributes(itemAttributes);
        /**
         dmg system
         */
