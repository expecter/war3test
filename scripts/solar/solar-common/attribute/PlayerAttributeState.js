...
export default class PlayerAttributeState {
     ...
     */
    static addInheritHandlers(inheritHandler) {
    ...
    static checkInherit(unitHandle) {
     ...
     */
    static refreshPlayerAttributes2PlayerUnits(player, playerAttribute) {
        if (playerAttribute == null) { //已有玩家的属性不能再直接把玩家属性设置为null 否则不会进入下面的单位属性刷新 导致单位继承的玩家属性没有清除
            //退出以节省运算性能 防止对进攻怪物进行属性判断
            return;
        }
        SelectUtil.forPlayerAliveUnits(unitHandle => {
    ...
    static refreshPlayerAtrr2UnitAtrr(unitHandle, playerAttribute) {
        let unitOldPlayerSAttr = DataBase.getUnitSolarData(unitHandle, false)?._SL_inheritPSA;
        /**
         */
