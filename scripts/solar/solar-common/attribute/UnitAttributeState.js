...
export default class UnitAttributeState {
     ...
     * @param attribute
     */
    static refreshUnitSolarAttribute(unitHandle, attribute) {
        ...
        if (attribute.attack_p) { //攻击 计算绿字
            ...
            if (val > -210000_0000 && val < 2100000000) { //不是大数值走整数 大数值才能走实数模拟 走整数可以防止小数点浮动
        ...
        if (attribute.life_p) {
            ...
            if (val > -210000_0000 && val < 2100000000) {
                val = Math.floor(val);
            }
        ...
        //
        if (attribute.primary_property || attribute.primary_property_p) {
            let primaryTypeKey = HeroUtil.getHeroPrimary(unitHandle);
            if (primaryTypeKey == "STR") {
                strength += attribute.primary_property || 0;
                strength_p += attribute.primary_property_p || 0;
            }
            else if (primaryTypeKey == "AGI") {
                agility += attribute.primary_property || 0;
                agility_p += attribute.primary_property_p || 0;
            }
            else if (primaryTypeKey == "INT") {
                intelligence += attribute.primary_property || 0;
                intelligence_p += attribute.primary_property_p || 0;
            }
        }
        //
