...
export default class SolarDataClearState {
    ...
    constructor(delay = SolarDataClearState.config.delay) {
            ...
            se.playerChat("-c", () => {
                BaseUtil.runLater(1, (count, maxCount) => {
                    if (count == maxCount) {
                        ...
                        DisplayTimedTextToPlayer(GetLocalPlayer(), 0, 0, 10, "|cffff0000【系统提示】清理地面物品完毕!");
                    }
                    else {
                        DisplayTimedTextToPlayer(GetLocalPlayer(), 0, 0, 10, "|cffff0000【系统提示】清理地面物品倒计时:" + (maxCount - count));
                    }
                }, 6, true);
            });
        }
    ...
    static _sl_hookClearHandle() {
        ...
        _G.RemoveItem = function (whichItem) {
            if (IsHandle(whichItem)) {
                let actor = DataBase.getItemSolarData(whichItem, false)?._SL_solarActorItem;
                if (actor) {
                    actor.destroy(true);
                }
