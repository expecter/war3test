...
export default class SolarBuffState {
     ...
     * 模拟buff的 ui显示
     */
    static objectPool = new ObjectPool(() => {
    ...
    static frameBuffMap = {};
    static ubertipFrameBD = null;
    static ubertipFrameText = null;
    constructor() {
     ...
     * @param unitHandle
     */
    static refreshBuffAttributes2UnitSolarAttribute(unitHandle) {
        //死亡单位不刷新
        if (!UnitStateUtil.isAlive(unitHandle)) {
            return;
        }
        let attributes = BuffUtil.getUnitBuffAttributes(unitHandle);
        let unitOldBuffsAttr = DataBase.getUnitSolarData(unitHandle, false)?._SL_totalBuffsSolarAttribute;
        if (unitOldBuffsAttr == null) { //如果之前没有旧buff属性 则退出后续逻辑  否则还需将之前的旧属性 移除
            if (attributes == null || attributes.length == 0) {
                return;
            }
        }
        let totalAttribute = AttributeUtil.sumAttributes(attributes);
        /**
         dmg system
         */
