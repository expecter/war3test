...
export default class SceneUtil {
    static _sl_ground_tiles_id = null;
     ...
     */
    static loadScene(scenePath, toCenterX, toCenterY, isFull = false) {
     ...
     * @param height
     */
    static setSceneBounds(sceneData) {
        //x
    ...
    static _sl_loadScene_ground_tiles_id(ground_tiles_idPath) {
    ...
    static _sl_loadScene_terrain_tile(csvPath, toCenterX, toCenterY, loadCliff = false) {
     ...
     * @private
     */
    static _sl_loadScene_terrain_cliff(csvPath, toCenterX, toCenterY) {
    ...
    static _sl_loadScene_terrain_route(terrain_route, toCenterX, toCenterY, isFull = false) {
    ...
    static _sl_loadScene_dood(doodPath, toCenterX, toCenterY) {
        let dood = CsvUtil.parse(doodPath);
        let cacheKey = "dood:" + toCenterX + ":" + toCenterY;
        let destCacheKey = "dest:" + toCenterX + ":" + toCenterY;
        let currentEffects = SceneUtil.cache.get(cacheKey);
        let currentDests = SceneUtil.cache.get(destCacheKey);
        //清除上次在这个区域加载的 地形装饰物
    ...
    static _sl_loadScene_terrain_unit(csvPath, toCenterX, toCenterY) {
        let csvData = CsvUtil.parse(csvPath);
        let cacheKey = "unit:" + toCenterX + ":" + toCenterY;
        let currentHandle = SceneUtil.cache.get(cacheKey);
        //清除上次在这个区域加载的 地形装饰物
