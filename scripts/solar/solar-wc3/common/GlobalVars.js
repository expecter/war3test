...
const globals = require('jass.globals');
const japi = require('jass.japi');
const ai = require('jass.ai');
const slk = require('jass.slk');
const runtime = require('jass.runtime');
const jConsole = require('jass.console');
const jDebug = require('jass.debug');
const jMessage = require('jass.message');
const storm = require('jass.storm');
_G.log = new Log();
export default class GlobalVars {
    static isInit = false;
    static _sl_isInitOnMain = false;
    static justInvoke = GlobalVars.init0();
    //要调一个函数 以免import GlobalVars 被编译器取消了
    /**
     * @param debug 是否调试模式
     * @param egp_enable 是否启用太阳编辑器游戏插件  (是否打开游戏左上角太阳Logo按钮) 平台测试异步时最好关闭此项 以免因版本不一致导致异步
     */
    static init(debug = isDebug, egp_enable = debug) {
    ...
    static init0() {
        ...
        setmetatable(_ENV, {
            __index: function (_, key) {
                let result = japi[key] || CJ[key] || ai[key] || jDebug[key] || jMessage[key] || globals[key] || DzApiHelper[key] || KKApiHelper[key];
                if (result) {
                    _ENV[key] = result;
                }
                return result;
    ...
    static initBaseVars() {
        ...
        // let ActorTypeBuildUtil = _require("solar.solar-common.actor.util.ActorTypeBuildUtil").default
        // //@ts-ignore
        // _G._sl_funs = {
        //     setBuilds2unit: ActorTypeBuildUtil.setBuilds2unit
        // }
    }
}
