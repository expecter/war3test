...
export default class ActorUnitUtil {
     ...
     */
    static createUnit(player: player, actorUnitTypeId: string, x: number, y: number, face: number = 0, count: number = 1, callBack?: (unit: unit) => void): unit {
        if (ActorTypeUtil.hasActorType(actorUnitTypeId)) {
            let actorCallBack: (actorUnit: ActorUnit) => void = null;
            if (callBack) {
                actorCallBack = (actorUnit: ActorUnit) => {
     ...
     * @param unit
     */
    static getUnitId(unit: unit): string {
        let actor: ActorUnit = DataBase.getUnitSolarData(unit, false)?._SL_solarActorUnit;
        if (actor == null) {
     ...
     * @param actorUnitType
     */
    static getPlayerActorUnits(playerIndex: number, actorUnitType?: string): ActorUnit[] {
        let resultUnits = []
        ...
        for (let i = 0; i <= 1000000; i++) {
            ...
            let actor: ActorUnit = DataBase.getUnitSolarData(unitHandle, false)?._SL_solarActorUnit;
