...
export default class DebugUtil {
    ...
    static openShop(...keys: string[]) {
        ...
        //
        _G.DzAPI_Map_GetMallItemCount = function (this:void,whichPlayer: player, key: string) {
            if (keys.includes(key)) {
                return 100;
            } else {
                return 0;
     ...
     * 模拟平台后台逻辑服务 如随机只读存档
     */
    static startBackendLogicServer() {
        if (!isDebug) {
            log.errorWithTraceBack("非测试环境不能调用此api。")
            return
        }
        PlatUtil.removeBackendLogicResult = (whichPlayer: player, key: string) => {
            ArchiveUtil.set(whichPlayer, "_sldbug_rn_" + key, null);
            return true;
        };
        PlatUtil.getBackendLogicIntResult = (whichPlayer: player, key: string) => {
            return tonumber(ArchiveUtil.get(whichPlayer, "_sldbug_rn_" + key)) ?? 0;
        };
        PlatUtil.checkBackendLogicExists = (whichPlayer: player, key: string) => {
            return ArchiveUtil.get(whichPlayer, "_sldbug_rn_" + key) != null;
        };
        _G.KKApiRandomSaveGameCount = (whichPlayer: player, groupkey: string) => {
            return 2100000000;
        }
        PlatUtil.requestBackendLogic = (whichPlayer: player, key: string, groupkey: string) => {
            if (ArchiveUtil.get(whichPlayer, "_sldbug_rn_" + key)) {
                //已经存在了 需要先移除才能重新生成随机数
                return false;
            }
            BaseUtil.runLater(1.1, () => {
                ArchiveUtil.set(whichPlayer, "_sldbug_rn_" + key, GetRandomInt(-200000000, 2000000000));
                let callBacks: SolarTrigger[] = PlatUtil['_sl_onBackendLogicUpdateEventCallBacks'][key];
                if (callBacks) {
                    let newRandomInt = PlatUtil.getBackendLogicIntResult(whichPlayer, key)
                    let timeStamp = 0
                    let groupKey = "1";
                    let data = {whichPlayer, newRandomInt, key, groupKey, timeStamp}
     ...
     * 打开商城与存档
     */
    static openShopArchiveMapLv(shop = 0, Archive = 0, mapLv = 0, env = _G) {
        if (Archive > 0) {
        ...
        if (shop > 0) {
            env.DzAPI_Map_HasMallItem = function (this: void, whichPlayer: player, key: string) {
                return true;
            }
            env.DzAPI_Map_GetMallItemCount = function (this: void, whichPlayer: player, key: string) {
                return shop;
            }
        }
        if (mapLv && mapLv > 0) {
            env.DzAPI_Map_GetMapLevel = function (this: void, whichPlayer: player) {
                return mapLv;
            }
        }


        env.DzAPI_Map_ContinuousCount = function (this: void, whichPlayer: player, id: number) {
            return 100;
        }
        env.DzAPI_Map_GetForumData = function (this: void, whichPlayer: player, id: number) {
            return 100;
        }
        env.DzAPI_Map_Returns = function (this: void, whichPlayer: player, id: number) {
            return true;
        }
        env.DzAPI_Map_GetGuildName = function (this: void, whichPlayer: player) {
            return "太阳公会";
        }
    }
