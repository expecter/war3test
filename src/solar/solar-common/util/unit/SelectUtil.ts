...
export default class SelectUtil {
     ...
     */
    static forEnemyUnitsInRect(center: unit, rect: rect, callback: (unit: unit) => void): void {
        let group = GroupUtil.groupObjectPool.borrowObject();
     ...
     * 可异步使用
     */
    static getUnitsInRange(x: number, y: number, radius: number): unit[] {
     ...
     * @param onlyAlive
     */
    static getPlayerUnits(playerIndex: number, unitType?: string, onlyAlive?: boolean): unit[] {
        let resultUnits = []
        let group = GroupUtil.groupObjectPool.borrowObject();
        GroupEnumUnitsOfPlayer(group, Player(playerIndex), null)
        if (unitType) {
