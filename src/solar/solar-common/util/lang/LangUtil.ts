...
export default class LangUtil {
     ...
     * 判断对象的key数量
     */
    static getObjectKeyCount(obj: any): number {
        let count = 0;
        for (let i in obj) {
            count++;
        }
     ...
     * 清空对象
     */
    static clearObject(obj: any): boolean {
        if (obj) {
            for (let k in obj) {
                obj[k] = null;
                delete obj[k];
     ...
     * @param compareFn
     */
    static forEachSort(set: any, callback: (key: any, val: any) => void, compareFn?: (a: string, b: string) => number) {
        let keys: string[] = []
        for (let key in set) {
            keys.push(key);
        }
