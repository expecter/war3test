...
export default class ArchiveUtil {
     ...
     * @param value
     */
    static updateMaxNumber(whichPlayer: player, key: string, value: number): number {
     ...
     * @param addValue
     */
    static addNumber(whichPlayer: player, key: string, addValue: number): number {
        let oldVal: number = ArchiveUtil.get(whichPlayer, key) || 0;
     ...
     * @deprecated
     */
    static saveAll(whichPlayer: player, ignoreWarning: boolean = false): void {
    ...
    private static _sl_saveBucket(whichPlayer: player, bucketId: string, bucket: any): void {
        ...
        let useBatch: boolean = !isDebug;//正式环境用批量保存 本地用基础保存才能保存到本地存档文件。
